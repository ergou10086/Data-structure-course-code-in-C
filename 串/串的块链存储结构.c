...
#define CHUNKSIZE 4 /* 可由用户定义的块大小,链表每一块存几个字符 */
typedef struct Chunk
{
    char ch[CHUNKSIZE];   // 字符数组，存储实际的字符串内容
    struct Chunk *next;   // 指向同类型（Chunk 类型）的下一个结构体，用于将多个 Chunk 结构体连接起来
}Chunk;
typedef struct
{
    Chunk *head, *tail; // head 指针指向整个链式存储结构的第一个 Chunk 结构体，它是表示字符串的起始位置，tail 指针指向链式存储结构的最后一个 Chunk 结构体
    int curlen;  // 记录当前字符串的长度
}LString;
